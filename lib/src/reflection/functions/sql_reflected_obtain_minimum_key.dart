...
class SqlReflectedObtainMinimumKey<T> with FunctionalityMixin<int> {
  final SqlEngine engine;
  final SqlReflectedClassStructure<T> reflectedStructure;

  const SqlReflectedObtainMinimumKey({required this.engine, required this.reflectedStructure});
  ...
  FutureResult<int> runInternalFuncionality() async {
    ...
    final query = SqlQueryCommand(
      columns: [ColumnSelection(columnName: primaryKeyField.name, function: ColumnSelectionFunction.minimum)],
